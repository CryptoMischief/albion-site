...
const categories = [
  ...
  { key: 'robotics', img: '/cats/robotics.jpg' },
        ...
        <div className="mt-10 grid gap-4 sm:mt-12 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map(({ key, img }) => (
            <Link
              ...
              className="group relative isolate flex min-h-60 flex-col justify-end overflow-hidden rounded-2xl shadow-sm transition-shadow hover:shadow-lg sm:min-h-72"
            >
              <Image
                src={img}
                alt=""
              ...
              />
              {/* bottom-up scrim keeps the text legible while the photo stays visible */}
              <div className="absolute inset-0 -z-10 bg-gradient-to-t from-navy-900/95 via-navy-900/40 to-transparent" />
              <div className="p-5 sm:p-6">
                <h3 className="text-lg font-semibold text-white drop-shadow">
                  {t(`categories.${key}.title`)}
                </h3>
                <p className="mt-1.5 max-w-[22rem] text-sm leading-relaxed text-white/85 drop-shadow">
                  {t(`categories.${key}.body`)}
                </p>
                <span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-white/90 drop-shadow">
                  {t('viewProducts')}
